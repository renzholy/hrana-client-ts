...
function handlePipelineResponse(
  pipeline: Array<PipelineEntry>,
  respBody: proto.PipelineRespBody
): void {
...
async function decodePipelineResponse(
  ...
  if (encoding === "json") {
    ...
    return readProtobufMessage(
      new Uint8Array(respData),
      protobuf_PipelineRespBody
    );
  } else {
    throw impossible(encoding, "Impossible encoding");
  }
